...
import { filterPresetEntries, imagePresetEntries, IMAGE_ACCEPT } from "@/config/presets.ts";
...
const LIGHT_SLIDERS: SliderDef[] = [
	...
	{ key: "brightness", label: "Brightness", min: -0.5, max: 0.5, step: 0.01, format: (v) => `${v >= 0 ? "+" : ""}${(v * 100).toFixed(0)}` },
	{ key: "contrast", label: "Contrast", min: 0.2, max: 3, step: 0.01, format: (v) => `${(v * 100).toFixed(0)}` },
	{ key: "highlights", label: "Highlights", min: -1, max: 1, step: 0.01, format: (v) => `${v >= 0 ? "+" : ""}${(v * 100).toFixed(0)}` },
	{ key: "shadows", label: "Shadows", min: -1, max: 1, step: 0.01, format: (v) => `${v >= 0 ? "+" : ""}${(v * 100).toFixed(0)}` },
];

const COLOR_SLIDERS: SliderDef[] = [
	{ key: "saturation", label: "Saturation", min: 0, max: 3, step: 0.01, format: (v) => `${(v * 100).toFixed(0)}` },
	{ key: "temperature", label: "Temperature", min: -1, max: 1, step: 0.01, format: (v) => `${v >= 0 ? "+" : ""}${(v * 100).toFixed(0)}` },
	{ key: "tint", label: "Tint", min: -1, max: 1, step: 0.01, format: (v) => `${v >= 0 ? "+" : ""}${(v * 100).toFixed(0)}` },
...
export function ImageSidebar({ processFn, wasmReady, onOpenFile, onNew }: ImageSidebarProps) {
	const {
		file, originalData, filteredData, filters,
		exportFormat, exportQuality,
		resizeWidth, resizeHeight, resizeLockAspect,
		setFilter, setSliderDragging, commitFilters,
		applyFilterPreset, resetFilters,
		setExportFormat, setExportQuality, setShowOriginal,
		setResizeWidth, setResizeHeight, setResizeLockAspect,
		applyResize,
	} = useImageEditorStore();
	...
	return (
		<aside className="w-72 xl:w-80 shrink-0 min-h-0 overflow-y-auto border-l border-border bg-surface flex flex-col" style={{ overscrollBehavior: "contain" }}>
			{/* File */}
			<div className="p-4 border-b border-border">
				...
				{!wasmReady && (
					<p className="mt-1.5 text-[11px] text-accent animate-pulse-soft">
						Loading engine...
					</p>
				)}
			</div>
			...
			<div className="p-4 border-b border-border">
				<div className="flex items-center justify-between mb-3">
					{sectionHeader("presets", "Color Presets",
						<button
							...
							<div className="flex items-center gap-2">
								...
								<button
									...
									className={`mt-4 h-7 w-7 flex items-center justify-center rounded-md transition-colors cursor-pointer ${
										resizeLockAspect ? "text-accent bg-accent/10" : "text-text-tertiary hover:text-text"
									}`}
								>
