...
/**
 * Plays a video file into a canvas, with its sound. The audio player is the clock when there is
 * sound; pictures are decoded just ahead and drawn when their time comes. Without sound, the page
 * clock drives playback. Paused, the frame at the playhead is shown, so scrubbing shows pictures.
 */
export class MediaPlayer {
	...
	constructor(file: File) {
		...
		this.ready = this.open(file);
	}

	private async open(file: File): Promise<MediaDetails> {
		...
		if (video && (await video.canDecode())) this.sink = new VideoSampleSink(video);
		if (audioTrack) {
			const audio = new AudioPlayer(file);
			if (await audio.playable()) {
				audio.setPlan({ ranges: [{ start: 0, end: duration }], envelope: [] });
				audio.onTime = (time) => {
					this.onTime(time);
				};
		...
		const size = video ? { width: video.displayWidth, height: video.displayHeight } : null;
		return { duration, video: this.sink ? size : null, audio: this.audio !== null };
	}
	...
	private async playPictures(run: number, from: number) {
		...
		if (!sink) return;
		const samples = sink.samples(from);
		...
		void (async () => {
			...
			try {
				...
				while (this.stillRequest !== null && !this.disposed) {
					const at = this.stillRequest;
					this.stillRequest = null;
					// oxlint-disable-next-line no-await-in-loop
					const sample = await this.sink?.getSample(Math.max(at, 0));
					if (sample) {
