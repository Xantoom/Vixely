...
async function handleGif(msg: GifMessage): Promise<void> {
	...
	const scaleExpr = msg.height != null
		? `scale=${msg.width}:${msg.height}:flags=lanczos`
		: `scale=${msg.width}:-1:flags=lanczos`;
	...
	try {
		...
		const paletteArgs = [
			...
			"-vf", `${vfChain},palettegen=max_colors=${maxColors}:stats_mode=diff`,
			"palette.png",
		];
		...
		const gifArgs = [
			...
			...(msg.startTime != null ? ["-ss", String(msg.startTime)] : []),
			...(msg.duration != null ? ["-t", String(msg.duration)] : []),
			"-lavfi", `${vfChain}[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=5`,
			"output.gif",
		];
