...
pub fn new(settings: Settings) -> GifResult<(Collector, Writer)> {
    ...
    // Vixely: WebAssembly has no threads, so every frame is collected before writing starts.
...
impl Writer {
    ...
    fn write_frames(&self, write_queue: Receiver<FrameMessage>, writer: &mut dyn Write, reporter: &Mutex<Option<&mut dyn ProgressReporter>>) -> CatResult<()> {
        ...
        #[cfg(target_arch = "wasm32")]
        let (lzw_queue, lzw_recv) = ordered_channel::unbounded();
    ...
    fn write_inner(&self, decode_queue_recv: Receiver<InputFrame>, writer: &mut dyn Write, reporter: &mut dyn ProgressReporter) -> CatResult<()> {
        ...
        // Vixely: without threads, the stages run one after the other, each draining the previous.
        #[cfg(target_arch = "wasm32")]
        {
            let (diff_queue, diff_queue_recv) = ordered_channel::unbounded();
            let res1 = self.make_resize(decode_queue_recv, diff_queue);
            let (quant_queue, quant_queue_recv) = crossbeam_channel::unbounded();
            let res2 = self.make_diffs(diff_queue_recv, quant_queue, reporter);
            let (remap_queue, remap_queue_recv) = ordered_channel::unbounded();
            let res3 = self.quantize_frames(quant_queue_recv, remap_queue);
            let (write_queue, write_queue_recv) = crossbeam_channel::unbounded();
            let res4 = self.remap_frames(remap_queue_recv, write_queue);
            let res0 = self.write_frames(write_queue_recv, writer, reporter);
            return combine_res(combine_res(combine_res(res0, res1), combine_res(res2, res3)), res4);
        }
...
#[cold]
#[cfg_attr(target_arch = "wasm32", allow(dead_code))] // Vixely: unused without threads.
